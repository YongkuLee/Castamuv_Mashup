...
@Controller
public class Youtube2Contoller {
	...
	public String search(@RequestParam(required = false) String text,
			@RequestParam(required = false) String channelId, Model model)
			throws IOException {

		List<Channel> list = service
				.channels(new UserId(DefaultValues.STORE_ID));
